/*
 * "$Id: image.c,v 1.1.2.1 2003/01/05 04:23:44 rlk Exp $"
 *
...
int
stp_image_depth(stp_image_t *image)
{
  return image->image_depth(image);
}

stp_color_mode_t
stp_image_color_mode(stp_image_t *image)
{
  return image->color_mode(image);
}
